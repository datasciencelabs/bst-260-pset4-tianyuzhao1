...
library(tidyverse)
library(janitor)
population <- as.data.frame(do.call(rbind, population)) # bind the list elements into a data frame 
population <- row_to_names(population, row_number = 1) # Use janitor row to names function
names(population)
...
population <- population |>   
  ...
  mutate(
    ...
    abbreviation = case_when(
      TRUE ~ state.abb[match(state_name, state_name)], # add state abbreviations using state.abb variable
...
population |> 
  ...
  mutate(state_name = reorder_within(state_name, population, year)) |> # reorder state
  ggplot(aes(x = state_name, y = population, fill = year)) + # assign aesthetic mapping
  geom_col() + # use geom_col to plot barplot
  coord_flip() + # flip coordinates
  facet_wrap(~year, ncol = 2, scales = "free_x") + # facet by year
  scale_x_reordered() +
  labs(
    title = "US Populations by State (2020 vs 2021)",
    x = "State",
    y = "Population"
  ) +
  theme_minimal()
...
library(purrr)
url <- "https://github.com/datasciencelabs/2025/raw/refs/heads/main/data/regions.json"
regions_json <- fromJSON(url) #use jsonlit JSON parser 
regions <- map_df(seq_along(regions_json$region), function(i){ # 
  tibble(
    region = regions_json$region[i],
    region_name = regions_json$region_name[i],
    state_name = regions_json$states[i]
  )
})
...
population <- population |>
  left_join(regions_flat, by = "state_name")
...
cdc_req <- request(api) 
cdc_resp <- req_perform(cdc_req)

cases_raw <- resp_body_json(cdc_resp, simplifyVector = TRUE) |>
  as_tibble()
...
No. Only 1000 rows of data are presented because the default limit for the API call is 1000 rows if not specified. 
...
pop_2020 <- population |>
  filter(year == "2020") |>
  select(state_name, abbreviation, population, region_name) |>
  rename(state=abbreviation)

cases <- cases_raw |>
  left_join(pop_2020, by = "state") |>
  filter(year(date) %in% c(2020, 2021)) |>
  mutate(
    cases_per_100k = (cases / population) * 100000
  )

ggplot(cases, aes(x = date, y = cases_per_100k, color = state)) +
  geom_line(alpha = 0.6, linewidth = 0.8) +
  facet_wrap(~ region_name, ncol = 3, scales = "free_y") +
  labs(
    title = "Weekly COVID-19 Cases per 100, 000 People by US REgion in 2020 and 2021",
    x = "Date",
...
kable(
  cases_monthly,
  caption = "Total COVID-19 Cases by Month in 2020 and 2021",
  digits = 0,
