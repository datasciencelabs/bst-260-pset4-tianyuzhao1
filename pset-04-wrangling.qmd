...
population <- population |>   
  ...
  mutate(
    ...
    population = as.numeric(population, year), # parese all relevant colunns to numeric
...
library(tidytext)
population |> 
  ggplot(aes(x = reorder(state_name, population), y = population, fill = year)) + # assign aesthetic mapping
...
cases <- cases_raw |>
  ...
  right_join(population |> mutate(year = as.numeric(year)) |>
               select(abbreviation, year, population, region_name),
             by = c("state" = "abbreviation", "year" = "year")) |>
  mutate(
